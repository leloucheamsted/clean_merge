...
/// Returned by [DataConnectionCheckService.connectionStatus]
enum DataConnectionStatus {
  disconnected,
  connected,
}

enum DisconnectionStatus { noConnectivity, noInternet, noService }
...
class DataConnectionCheckService implements IConnectivityService {
  ...
  /// The NetworkJitterController needs the latency info from each network check
  Function(int)? processLatency;

  /// This is a singleton that can be accessed like a regular constructor
  /// i.e. DataConnectionChecker() always returns the same instance.
  factory DataConnectionCheckService() => _instance;
  ...
  DisconnectionStatus? _disconnectionStatus;

  DisconnectionStatus? get disconnectionStatus => _disconnectionStatus;

  AddressCheckOptions2? _serviceAddress;

  // ignore: unnecessary_getters_setters
  AddressCheckOptions2? get serviceAddress => _serviceAddress;

  int get recoveryDurationInSeconds => _recoveryDurationInSeconds;

  // ignore: unnecessary_getters_setters
  set serviceAddress(AddressCheckOptions2? value) => _serviceAddress = value;

  int _recoveryDurationInSeconds = 2;
  ...
  Future<bool> get hasConnection async {
    ...
    if (serviceAddress == null) {
      ...
      try {
        ...
        // TelloLogger().i(" ${response.request!.url} response.statusCode == ${response.statusCode} ");
        result = response.statusCode == 200;
        if (!result) {
          _disconnectionStatus = DisconnectionStatus.noService;
        }
      } catch (err) {
        result = result && false;
        _disconnectionStatus = DisconnectionStatus.noService;
  ...
  /// Initiates a request to each address in [addresses].
  /// If at least one of the addresses is reachable
  /// we assume an internet connection is available and return `true` otherwise.
  Future<bool> get isConnectedToInternet async {
    ...
    if (connectivityStatus == ConnectivityStatus.none || connectivityStatus == ConnectivityStatus.unknown) {
  ...
  ///
  /// Defaults to [DEFAULT_INTERVAL] (5 seconds).
  Duration checkInterval = DEFAULT_INTERVAL;
  int _disconnectDurationInSeconds = 0;

  int get disconnectDurationInSeconds => _disconnectDurationInSeconds;
  ...
  _maybeEmitStatusUpdate([Timer? timer]) async {
    ...
    _timerHandle?.cancel();
    timer?.cancel();
    DataConnectionStatus currentStatus = DataConnectionStatus.connected;
    /* final ConnectivityStatus connectivityStatus = await Connectivity().checkConnectivity();
    if (connectivityStatus == ConnectivityStatus.none || connectivityStatus == ConnectivityStatus.unknown) {
      currentStatus = DataConnectionStatus.disconnected;
      _disconnectionStatus = DisconnectionStatus.noConnectivity;
    } else {*/
    currentStatus = await connectionStatus;
    //}
    ...
    if (currentStatus == DataConnectionStatus.disconnected) {
      _disconnectDurationInSeconds += checkInterval.inSeconds;
    }
    // update last status
...
/// default to [DataConnectionCheckService.DEFAULT_PORT]
/// and [DataConnectionCheckService.DEFAULT_TIMEOUT]
...
class AddressCheckOptions {
  ...
  AddressCheckOptions(
    this.address, {
    this.port = DataConnectionCheckService.DEFAULT_PORT,
    this.timeout = DataConnectionCheckService.DEFAULT_TIMEOUT,
  });
...
class AddressCheckOptions2 {
  final String? address;
  final String method;
  final Duration timeout;

  AddressCheckOptions2(
    this.address, {
    this.method = "",
    this.timeout = DataConnectionCheckService.DEFAULT_TIMEOUT,
  });
  ...
  @override
  String toString() => "AddressCheckOptions($address, $method, $timeout)";
}
